 ...
 */
$result = $gaclApi->add_object_section('Jedi', 'jedi', 12, 0, 'ARO'); //Must specifiy Object Type, notice it is ARO now.
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->get_group_id('passengers');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group('jedi','Jedi', $passengersGroupID, 'aro');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 * edit_object($objectId, $sectionValue, $name, $value=0, $order=0, $hidden=0, $objectType)
 */

/*
 * First we need the object_id's!
 * get_object_id($sectionValue, $value, $objectType)
 */
$result = $gaclApi->get_object_id('passengers','obi-wan','aro');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->get_object_id('passengers','luke','aro');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 *
 * get_group_id($value, $name, $groupType)
 * del_group_object($groupId, $object_section_value, $object_value, $groupType)
 * add_group_object($groupId, $object_section_value, $object_value, $groupType)
 */

/*
 * First lets get the Passengers ARO GroupID
 */
$result = $gaclApi->get_group_id('passengers', 'Passengers', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->del_group_object($passengersGroupId, 'passengers', 'obi-wan', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->del_group_object($passengersGroupId, 'passengers', 'luke', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->edit_object($obiWanObjectId, 'jedi', 'Obi-wan', 'obi-wan', 10, 0, 'aro');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
unset($result);
$result = $gaclApi->edit_object($lukeObjectId, 'jedi', 'Luke', 'luke', 11, 0, 'aro');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->get_group_id('jedi', 'Jedi', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group_object($jediGroupId, 'jedi', 'obi-wan', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->add_group_object($jediGroupId, 'jedi', 'luke', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$aro_array_GroupID =array($gaclApi->get_group_id('jedi') );
$allow = TRUE;
$enabled = TRUE;
$return_value = NULL;
...
if ( $gaclApi->acl_check('access', 'lounge', 'jedi', 'obi-wan') ) {
	if ($outputDebug == TRUE){
...
if ( $gaclApi->acl_check('access', 'cockpit', 'jedi', 'obi-wan') ) {
	if ($outputDebug == TRUE){
...
if ( $gaclApi->acl_check('access', 'engines', 'jedi', 'obi-wan') ) {
	if ($outputDebug == TRUE){
