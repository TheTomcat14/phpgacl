 ...
 *
 * add_object_section($name, $value=0, $order=0, $hidden=0, $objectType=NULL)
 */

$result = $gaclApi->add_object_section('Access', 'access', 10, 0, 'ACO'); //Must specifiy Object Type.
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 *		- Cockpit
 *
 * add_object($sectionValue, $name, $value=0, $order=0, $hidden=0, $objectType=NULL)
 */
$result = $gaclApi->add_object('access', 'Lounge', 'lounge', 10, 0, 'ACO'); //Must specifiy Object Type.
if ($outputDebug == TRUE)
{
	if ($result !== FALSE) {
...
// Add the Engines ACO
$result = $gaclApi->add_object('access', 'Engines', 'engines', 10, 0, 'ACO'); //Must specifiy Object Type.
if ($outputDebug == TRUE)
{
	if ($result !== FALSE) {
...
// Add the Guns ACO
$result = $gaclApi->add_object('access', 'Guns', 'guns', 10, 0, 'ACO'); //Must specifiy Object Type.
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
// Add the Cockpit ACO
$result = $gaclApi->add_object('access', 'Cockpit', 'cockpit', 10, 0, 'ACO'); //Must specifiy Object Type.
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 * add_object_section($name, $value=0, $order=0, $hidden=0, $objectType=NULL)
 */
$result = $gaclApi->add_object_section('Crew', 'crew', 10, 0, 'ARO'); //Must specifiy Object Type, notice it is ARO now.
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->add_object_section('Passengers', 'passengers', 11, 0, 'ARO'); //Must specifiy Object Type, notice it is ARO now.
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
// Add Han to the Crew
$result = $gaclApi->add_object('crew', 'Han', 'han', 10, 0, 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->add_object('crew', 'Chewie', 'chewie', 11, 0, 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->add_object('passengers', 'Obi-wan', 'obi-wan', 10, 0, 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->add_object('passengers', 'Luke', 'luke', 11, 0, 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->add_object('passengers', 'R2D2', 'r2d2', 12, 0, 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$result = $gaclApi->add_object('passengers', 'C3PO', 'c3po', 13, 0, 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 * '-Passengers Group
 *
 * add_group($value, $name, $parentId, $groupType);
 */
 ...
 */
$result = $gaclApi->add_group('millennium_falcon_passengers','Millennium Falcon Passengers', 0, 'aro');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group('crew','Crew', $millenniumFalconPassengersGroupID, 'aro');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group('passengers','Passengers', $millenniumFalconPassengersGroupID, 'aro');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 *   '-C3PO ARO
 *
 * add_group_object($groupId, $object_section_value, $object_value, $groupType='ARO')
 */

/*
 * Assign Han to the Crew Group.
 */
$result = $gaclApi->add_group_object($crewGroupID, 'crew', 'han', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group_object($crewGroupID, 'crew', 'chewie', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group_object($passengersGroupID, 'passengers', 'obi-wan', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group_object($passengersGroupID, 'passengers', 'luke', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group_object($passengersGroupID, 'passengers', 'r2d2', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 */
$result = $gaclApi->add_group_object($passengersGroupID, 'passengers', 'c3po', 'ARO');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
 ...
 *   '-C3PO ARO
 *
 * addAcl($aco_array, $aro_array, $aro_group_ids=NULL, $axo_array=NULL, $axo_group_ids=NULL, $allow=1, $enabled=1, $return_value=NULL, $note=NULL, $sectionValue=NULL )
 */
...
$aro_array_GroupID =array($gaclApi->get_group_id('crew') );
$allow = TRUE;
$enabled = TRUE;
$return_value = NULL;
...
$result = $gaclApi->addAcl($aco_array, NULL, $aro_array_GroupID, NULL, NULL, $allow, $enabled, $return_value, $note, 'user');
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
...
$aro_array_GroupID = array($gaclApi->get_group_id('Passengers'));
$allow = TRUE;
$enabled = TRUE;
$return_value = NULL;
...
//The NULL values are for the more advanced options such as groups, and AXOs. Refer to the manual for more info.
$result = $gaclApi->addAcl($aco_array, NULL, $aro_array_GroupID, NULL, NULL, $allow, $enabled, $return_value, $note);
if ($outputDebug == TRUE){
	if ($result !== FALSE) {
